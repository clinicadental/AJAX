<?php
    ...
    $string="SELECT cita.id,cliente.nombre AS clinombre,cliente.apellidos AS cliapellidos,dentista.nombre AS denombre,dentista.apellidos AS deapellidos,pago.importe,cita.fechacita,cita.procedimiento,cita.descripcion,cita.sala,cita.atendida FROM cita,cliente,dentista,pago WHERE cita.idcliente=cliente.id AND cita.iddentista=dentista.id AND cita.idpago=pago.id ORDER BY cita.fechacita ASC ";
    $datos=array();
